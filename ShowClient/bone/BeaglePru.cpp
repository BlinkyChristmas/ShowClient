...
auto BeaglePru::setData(const std::uint8_t *ptrToData, int length, int offsetInPruMemory) -> bool {
    ...
    if (this->mapped_address == nullptr || (length + offsetInPruMemory >= PRUMAPSIZE)) {
        std::cerr << "Was asked an invalid thing to write: Mapped address null? "s << std::to_string(this->mapped_address == nullptr) << " Length was: "s << std::to_string(length) << " Offset was: "s << std::to_string(offsetInPruMemory) << std::endl;
        return false ;
    }
